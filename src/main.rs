...
mod test {
    ...
    fn test_set_modified() {
        let src = PathBuf::from("/dev/shm/targetHEZ4FAC-MbjM0qJjFnRQp/");
        let dest = PathBuf::from("/home/ethan/ramdisk/cargo-ramdisk/target2");
        copy(src, dest).expect("Failed to copy");
    }
}
